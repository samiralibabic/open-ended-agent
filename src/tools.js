...
import { ensureDir, listRecursive, readTextSafe, resolveInside, writeText, appendText } from './fs_sandbox.js';

const WRITE_PREFIXES = ['workspace/', 'artifacts/'];
const READ_EXCLUDES = new Set(['node_modules', '.git']);
const TEXT_CONTENT_TYPES = ['text/', 'application/json', 'application/xml', 'application/xhtml+xml', 'application/rss+xml', 'application/atom+xml'];
...
function ensureWritableRel(rel) {
  ...
  if (!WRITE_PREFIXES.some(prefix => normalized === prefix.slice(0, -1) || normalized.startsWith(prefix))) {
    throw new Error(`Writes are restricted to ${WRITE_PREFIXES.join(', ')}. Requested: ${rel}`);
  }
  return normalized;
}
...
async function scanTextFiles(root, query, maxMatches = 50) {
  ...
  async function walk(dir, rel, depth) {
    ...
    for (const entry of entries) {
      ...
      } else if (/\.(md|txt|json|jsonl|js|ts|html|css|csv)$/i.test(entry.name)) {
        try {
...
function isLikelyPdfUrl(url) {
  try {
    const parsed = new URL(url);
    return /\.pdf($|[?#])/i.test(parsed.pathname) || parsed.hostname === 'arxiv.org' && parsed.pathname.startsWith('/pdf/');
  } catch {
    return false;
  }
}
...
async function fetchWithTimeout(url, options = {}) {
  ...
  try {
    ...
    if (err?.name === 'AbortError') throw new Error(`fetch timed out after ${config.fetchTimeoutMs}ms`);
    throw err;
  } finally {
    clearTimeout(timer);
  }
...
function urlToCacheSlug(url) {
  ...
  try {
    const parsed = new URL(url);
    name = parsed.pathname.replace(/\//g, '-').replace(/^-/, '').replace(/[^a-z0-9-]/gi, '-').slice(0, 40);
...
async function webSearch(query) {
  if (!config.webEnabled) throw new Error('web_search is disabled. Set AGENT_WEB=1 to enable.');
  ...
  return { query, results, note: results.length ? 'Search results parsed from DuckDuckGo HTML.' : 'No parsed results. Try a different query or fetch a known URL.' };
}

async function fetchUrl(url, artifactsDir) {
  if (!config.webEnabled) throw new Error('fetch_url is disabled. Set AGENT_WEB=1 to enable.');
  const parsed = new URL(url);
  ...
  const body = contentType.includes('html') || raw.trim().startsWith('<!DOCTYPE') || raw.trim().startsWith('<html')
    ? stripHtml(raw)
    : raw.replace(/\s+$/g, '');
...
function validateShellCommand(command) {
  if (!config.shellEnabled) {
    throw new Error('run_shell is disabled. Set AGENT_SHELL=1 to enable restricted shell commands.');
  }
  if (typeof command !== 'string' || !command.trim()) throw new Error('run_shell requires command.');
  const banned = [
    /\bsudo\b/, /\brm\b/, /\bmv\b/, /\bchmod\b/, /\bchown\b/, /\bkill\b/, /\bpkill\b/,
    /\bssh\b/, /\bscp\b/, /\bbrew\b/, /\bopen\b/, /\bosascript\b/,
    /\bcurl\b/, /\bwget\b/, /\bgit\s+push\b/, /\bgit\s+clone\b/,
    /\/Users\//, /\/etc\//, /\/System\//, /\.\./, /~\//
  ];
  for (const re of banned) {
    if (re.test(command)) throw new Error(`Command rejected by sandbox policy: ${re}`);
  }
  return command;
}
...
async function runShell(command, cwd) {
  ...
  return await new Promise((resolve) => {
    ...
    child.on('close', code => {
      clearTimeout(timer);
      resolve({ code, stdout: stdout.slice(0, 20000), stderr: stderr.slice(0, 20000), truncated: stdout.length > 20000 || stderr.length > 20000 });
    });
  });
}

export function summarizeResultForContext(result, maxChars = config.contextResultChars) {
  if (!result || typeof result !== 'object') return result;
  const data = result.data;
  if (!result.ok) return { ok: false, error: String(result.error || '').slice(0, 1200) };

  if (data?.tree && Array.isArray(data.tree)) {
    return { ok: true, data: { note: data.note, treeCount: data.tree.length, treeSample: data.tree.slice(0, 40) } };
  }
  if (Array.isArray(data)) {
    return { ok: true, data: data.slice(0, 80), omitted: Math.max(0, data.length - 80) };
  }
  if (data?.results && Array.isArray(data.results)) {
    return { ok: true, data: { query: data.query, results: data.results.slice(0, 8), note: data.note } };
  }
  ...
  return { ok: true, data: { summary: json.slice(0, maxChars), contextTruncated: true, originalChars: json.length } };
}

export async function executeAction(action, dirs) {
  try {
    if (!action || typeof action !== 'object') throw new Error('Missing action object.');
    const type = action.type;
    switch (type) {
      case 'observe': {
        const tree = await listRecursive(dirs.home, { maxEntries: config.observeMaxEntries, maxDepth: 4, exclude: ['node_modules', '.git'] });
        return ok({ tree, note: 'Observation includes a bounded sandbox file tree.' });
      }
      case 'list_dir': {
        const rel = normalizeRel(action.path || '.');
        const abs = resolveInside(dirs.home, rel);
        const entries = await fs.readdir(abs, { withFileTypes: true });
        return ok(entries.map(e => ({ name: e.name, type: e.isDirectory() ? 'dir' : 'file' })));
      }
      case 'read_file': {
        const rel = normalizeRel(action.path);
        const abs = resolveInside(dirs.home, rel);
      ...
      case 'read_file_range': {
        ...
        const length = Math.max(1, Math.min(Number(action.length ?? 12000), 50000));
      ...
      case 'write_file': {
        ...
        return ok({ path: rel, bytes: Buffer.byteLength(String(action.content ?? ''), 'utf8') });
      }
      case 'append_file': {
        const rel = ensureWritableRel(action.path);
        const abs = resolveInside(dirs.home, rel);
      ...
      case 'reflect': {
        const tree = await listRecursive(dirs.home, { maxEntries: Math.min(config.observeMaxEntries, 80), maxDepth: 4, exclude: ['node_modules', '.git'] });
        return ok({ note: 'Reflection action selected. Use this observation to assess repetition, current threads, and next useful exploration.', tree });
      }
      case 'sleep': {
