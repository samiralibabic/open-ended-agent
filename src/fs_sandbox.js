...
export function resolveInside(root, requested = '.') {
  ...
  if (path.isAbsolute(requested)) {
    throw new Error('Absolute paths are not allowed. Use paths relative to agent home.');
  }
...
export async function listRecursive(root, opts = {}) {
  ...
  async function walk(dir, rel, depth) {
    ...
    for (const entry of entries) {
      ...
      rows.push(`${'  '.repeat(depth)}${entry.isDirectory() ? 'dir ' : 'file'} ${childRel}${entry.isDirectory() ? '/' : ''}`);
  ...
  await walk(root, '', 0);
  if (rows.length >= maxEntries) rows.push(`[truncated at ${maxEntries} entries]`);
  return rows.join('\n');
}
