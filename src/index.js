...
function detectLoop(cycles, windowSize = 10) {
  ...
  if (fetchOrSearchCount >= windowSize * 0.8) {
    const hasMemoryUpdate = recent.some(c => {
      ...
      return Object.values(mem).some(v => v && (Array.isArray(v) ? v.length : true));
    });
    if (!hasMemoryUpdate) {
...
async function updateStateFile(dirs, cycles) {
  ...
  let focus = '';
  try {
    const ws = await readTextSafe(path.join(dirs.memory, 'working_summary.md'), 2000);
    const match = ws.match(/^# Working Summary\n+(.+)$/m);
    focus = match ? match[1].slice(0, 300).replace(/\n/g, ' ') : '';
  } catch {}
  
  // Recent actions
  const recentActions = recent.slice(0, 5).map(c => {
  ...
  try {
    ...
    artifactCount = files.filter(f => f.endsWith('.md') && !f.startsWith('.')).length;
  } catch {}
  ...
  try {
    ...
    if (wsFiles.filter(f => !f.startsWith('.')).length <= 1) risks.push('Workspace empty');
  } catch {}
  ...
  // Suggestions
  const suggestions = [];
  if (!hasSkills && oqCount > 3) suggestions.push('Capture research workflow as skill');
  if (artifactCount >= 2 && oqCount > 3) suggestions.push('Create synthesis artifact to answer open questions');
  if (risks.includes('Workspace empty')) suggestions.push('Create something in workspace/');
  if (loopWarning) suggestions.push('Consolidate findings before continuing research');
...
async function run() {
  ...
  console.log(`max_cycles=${config.maxCycles === 0 ? 'none' : config.maxCycles}`);
  ...
  while (true) {
    ...
    try {
      ...
      decision = {
        ...
        memory_updates: {
          mistakes_add: [`Harness/model error at cycle ${cycle}: ${String(err.message || err).slice(0, 500)}`]
        },
        action: { type: 'sleep', ms: 1000, reason: 'Cheap fallback after harness/model error; avoid adding more observation context after a failure.' }
      };
      result = { ok: false, error: String(err.stack || err.message || err).slice(0, 3000) };
      await applyMemoryUpdates(dirs, decision.memory_updates);
    }
    ...
    console.log(`[cycle ${cycle}] ${summarizeDecision(decision)} -> ${result?.ok ? 'ok' : 'error'}`);
    if (!result?.ok) console.log(`  ${result?.error || 'unknown error'}`);

    try {
      const memoryInfo = await getMemoryHealth(dirs);
      console.log(`  ctx=${memoryInfo.totalChars} chars (mem=${memoryInfo.memoryChars} logs=${memoryInfo.logsChars} tree=${memoryInfo.treeChars} obs=${memoryInfo.obsChars} artifacts=${memoryInfo.artifactChars})`);
    } catch {}
    ...
    if (cycle % 5 === 0) {
      try {
        const allCycles = [];
        try {
          const text = await readTextSafe(path.join(dirs.logs, 'cycles.jsonl'), 5_000_000);
          allCycles.push(...text.trim().split('\n').filter(Boolean).map(l => JSON.parse(l)));
        } catch {}
