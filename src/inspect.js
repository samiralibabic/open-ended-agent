...
async function inspectCycles(count = 10) {
  ...
  for (const c of cycles.reverse()) {
    ...
    console.log(`  Action: ${action.type} ${action.path || action.query ? `(${action.path || action.query?.slice(0,40)})` : ''}`);
    console.log(`  Result: ${r?.ok ? 'OK' : 'ERR: ' + String(r?.error || '').slice(0,60)}`);
    
    const mem = d?.memory_updates || {};
    const updated = Object.entries(mem).filter(([k,v]) => v && (Array.isArray(v) ? v.length : true));
    if (updated.length) {
...
async function inspectState() {
  ...
  const workingSummary = await readFileSafe(path.join(dirs.memory, 'working_summary.md'));
  const longTerm = await readFileSafe(path.join(dirs.memory, 'long_term.md'));
  const openQuestions = await readFileSafe(path.join(dirs.memory, 'open_questions.md'));
  ...
  for (const c of recent.slice(0, 5)) {
    ...
    console.log(`  C${c.cycle}: ${a.type} ${a.query ? `"${a.query.slice(0,40)}"` : a.url ? a.url.slice(0,40) : a.path || ''}`);
  }
  console.log('');
  
  // Open questions
  const oqLines = openQuestions.split('\n').filter(l => l.trim().startsWith('-'));
  ...
  const recentActions = recent.slice(0, 10).map(c => c.decision?.action?.type);
  const actionCounts = {};
  ...
  if (dominant && dominant[1] >= 6) {
    console.log(`  ⚠ Possible loop: ${dominant[0]} in ${dominant[1]}/10 recent cycles`);
  }
...
async function memoryDiff() {
  ...
  for (const c of cycles.reverse()) {
    ...
    if (mem.working_summary) lastWorking = { cycle: c.cycle, content: mem.working_summary };
    if (mem.long_term_add?.length) lastLongTerm = { cycle: c.cycle, items: mem.long_term_add };
    if (mem.open_questions_add?.length) lastOpenQ = { cycle: c.cycle, items: mem.open_questions_add };
    if (mem.skills_add?.length) lastSkills = { cycle: c.cycle, items: mem.skills_add };
  }
...
async function artifactIndex() {
  ...
  try {
    ...
    const artifacts = files.filter(f => f.endsWith('.md') && !f.startsWith('.'));
...
async function timeline() {
  ...
  for (const c of cycles) {
    ...
    // Detect milestones
    if (action === 'web_search' && !milestones.find(m => m.includes('first web search'))) {
      milestones.push(`C${c.cycle}: first web search`);
    }
    if (action === 'fetch_url' && !milestones.find(m => m.includes('first source'))) {
      milestones.push(`C${c.cycle}: first source fetch`);
    }
    if (action === 'write_file' && !milestones.find(m => m.includes('first artifact'))) {
      milestones.push(`C${c.cycle}: first artifact created`);
    }
    if (c.decision?.memory_updates?.skills_add?.length && !milestones.find(m => m.includes('skills'))) {
