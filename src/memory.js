...
import { ensureDir, exists, readTextSafe, appendText, writeText, listRecursive } from './fs_sandbox.js';
...
export async function readMemoryBundle(dirs, maxChars = 50000) {
  const specs = [
    ...
    { file: path.join(dirs.memory, 'working_summary.md'), cap: 12000, mode: 'head' },
    { file: path.join(dirs.memory, 'long_term.md'), cap: 18000, mode: 'tail' },
    { file: path.join(dirs.memory, 'open_questions.md'), cap: 10000, mode: 'tail' },
...
export async function getMemoryHealth(dirs) {
  ...
  const memoryBudget = Math.floor(budget * 0.40);
  const logsBudget = Math.floor(budget * 0.25);
  const treeBudget = Math.floor(budget * 0.10);
  const obsBudget = Math.floor(budget * 0.10);
  const artifactBudget = Math.floor(budget * 0.10);

  const memory = await readMemoryBundle(dirs, memoryBudget);
  const recentLogs = (await readRecentLogs(dirs, config.recentLogCycles)).slice(0, logsBudget);
  const artifactIndex = (await getArtifactIndex(dirs, artifactBudget)).slice(0, artifactBudget);

  const largestMemFile = (await Promise.all([
    { name: 'working_summary.md', path: path.join(dirs.memory, 'working_summary.md') },
    { name: 'long_term.md', path: path.join(dirs.memory, 'long_term.md') },
    { name: 'open_questions.md', path: path.join(dirs.memory, 'open_questions.md') },
...
export async function getArtifactIndex(dirs, maxChars = 4000) {
  ...
  for (const e of entries) {
    const sizeKb = e.size > 1024 ? `${(e.size / 1024).toFixed(1)}kb` : `${e.size}b`;
    const title = e.firstLine || e.name;
    lines.push(`- ${e.name} (${sizeKb})${e.date ? ` — ${e.date}` : ''}: ${title}`);
  }

  const index = lines.join('\n');
  return index.length <= maxChars ? index : index.slice(0, maxChars) + '\n[...more artifacts...]';
}
...
export async function latestObservation(dirs) {
  const logPath = path.join(dirs.logs, 'cycles.jsonl');
  if (!(await exists(logPath))) return 'No previous observation. This is the first cycle.';
  const text = await readTextSafe(logPath, 5_000_000);
  const lines = text.trim().split('\n').filter(Boolean);
  ...
  }, null, 2).slice(0, config.contextResultChars);
}
...
export async function applyMemoryUpdates(dirs, updates) {
  ...
  if (typeof updates.working_summary === 'string' && updates.working_summary.trim()) {
    await writeText(
  ...
  await appendUniqueSection(path.join(dirs.memory, 'long_term.md'), now, updates.long_term_add);
  await appendUniqueSection(path.join(dirs.memory, 'open_questions.md'), now, updates.open_questions_add);
  await appendUniqueSection(path.join(dirs.memory, 'skills.md'), now, updates.skills_add);
  await appendUniqueSection(path.join(dirs.memory, 'mistakes.md'), now, updates.mistakes_add);
}
...
export async function buildContext(dirs, cycle) {
  ...
  const memory = await readMemoryBundle(dirs, memoryBudget);
  const recentLogs = (await readRecentLogs(dirs, config.recentLogCycles)).slice(0, logsBudget);
  const tree = (await listRecursive(dirs.home, { maxEntries: Math.min(config.observeMaxEntries, 120), maxDepth: 4, exclude: ['node_modules', '.git'] })).slice(0, treeBudget);
  const previousObservation = (await latestObservation(dirs)).slice(0, Math.min(obsBudget, config.contextResultChars));
  const artifactIndex = (await getArtifactIndex(dirs, artifactBudget)).slice(0, artifactBudget);
  const configSummary = [
    `model=${config.model}`,
...
export async function maybeCompact(dirs, cycle) {
  ...
  const recentLogs = await readRecentLogs(dirs, Math.max(config.recentLogCycles, 20));
  ...
  await appendJsonl(path.join(dirs.logs, 'compactions.jsonl'), { time: new Date().toISOString(), cycle, raw, json });
  return json;
}
